...
import org.apache.hadoop.hbase.mapreduce.CompositeKeyImportTsv.TsvParser;
import org.apache.hadoop.hbase.mapreduce.CompositeKeyImportTsv.TsvParser.BadTsvLineException;
import org.apache.hadoop.hbase.mapreduce.CompositeKeyImportTsv.TsvParser.ParsedLine;
...
public class TestImportTsv {
  ...
  @Test
  public void testTsvParserSpecParsing() throws BadTsvLineException {
    TsvParser parser;
    ...
    parser = new TsvParser("HBASE_ROW_KEY_1", "\t");
    assertNull(parser.getFamily(0));
    assertNull(parser.getQualifier(0));
    assertEquals(0, parser.getRowKeyColumnIndex());
    assertFalse(parser.hasTimestamp());
  ...
  @Test
  public void testTsvParserWithCompositeKey() throws BadTsvLineException {
    TsvParser parser = new TsvParser("col_a,col_b:qual,HBASE_ROW_KEY_2,col_d,HBASE_ROW_KEY_1,col_f", "\t");
    assertBytesEquals(Bytes.toBytes("col_a"), parser.getFamily(0));
    assertBytesEquals(HConstants.EMPTY_BYTE_ARRAY, parser.getQualifier(0));
    assertBytesEquals(Bytes.toBytes("col_b"), parser.getFamily(1));
    assertBytesEquals(Bytes.toBytes("qual"), parser.getQualifier(1));
    assertNull(parser.getFamily(2));
    assertNull(parser.getQualifier(2));
    assertEquals(2, parser.getRowKeyColumnsIndex().size());
    assertEquals(2, (int)parser.getRowKeyColumnsIndex().get(1));
    assertEquals(4, (int)parser.getRowKeyColumnsIndex().get(0));

    assertEquals(TsvParser.DEFAULT_TIMESTAMP_COLUMN_INDEX, parser
        .getTimestampKeyColumnIndex());

    byte[] line = Bytes.toBytes("val_a\tval_b\tval_c\tval_d\tval_e\tval_f");
    ParsedLine parsed = parser.parse(line, line.length);
    checkParsing(parsed, Splitter.on("\t").split(Bytes.toString(line)));
    assertEquals("val_e-val_c", Bytes.toString(parsed.getRowKey().copyBytes()));
  }
  ...
  throws Exception {
    ...
    String[] args = new String[] {
        "-D" + CompositeKeyImportTsv.COLUMNS_CONF_KEY + "=HBASE_ROW_KEY,FAM:A,FAM:B",
        "-D" + CompositeKeyImportTsv.SEPARATOR_CONF_KEY + "=\u001b",
        TABLE_NAME,
        INPUT_FILE
    };
  ...
  public void testMROnTableWithTimestamp() throws Exception {
    ...
    // Prepare the arguments required for the test.
    String[] args = new String[] {
        "-D" + CompositeKeyImportTsv.COLUMNS_CONF_KEY
            + "=HBASE_ROW_KEY,HBASE_TS_KEY,FAM:A,FAM:B",
        "-D" + CompositeKeyImportTsv.SEPARATOR_CONF_KEY + "=,", TABLE_NAME, INPUT_FILE };
  ...
  throws Exception {
    ...
    // Prepare the arguments required for the test.
    String[] args = new String[] {
        "-D" + CompositeKeyImportTsv.MAPPER_CONF_KEY + "=org.apache.hadoop.hbase.mapreduce.TsvImporterCustomTestMapper",
        TABLE_NAME,
        INPUT_FILE
    };
  ...
  private void doMROnTableTest(String inputFile, String family, String tableName,
    ...
    try {
      ...
      if (conf.get(CompositeKeyImportTsv.BULK_OUTPUT_CONF_KEY) == null) {
  ...
  public void testBulkOutputWithoutAnExistingTable() throws Exception {
    ...
    String[] args = new String[] {
        "-D" + CompositeKeyImportTsv.COLUMNS_CONF_KEY + "=HBASE_ROW_KEY,FAM:A,FAM:B",
        "-D" + CompositeKeyImportTsv.SEPARATOR_CONF_KEY + "=\u001b",
        "-D" + CompositeKeyImportTsv.BULK_OUTPUT_CONF_KEY + "=output", TABLE_NAME,
        INPUT_FILE };
    doMROnTableTest(INPUT_FILE, FAMILY, TABLE_NAME, null, args, 3);
  }
